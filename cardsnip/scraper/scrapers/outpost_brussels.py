...
class OutpostProductParser(HTMLParser):
...
class OutpostBrusselsScraper(BaseScraper):
    scraper_key = "outpost_brussels"

    def scrape(self) -> ProductCheck:
        html = self._load_html()
        parser = OutpostProductParser()
        parser.feed(html)
    ...
    def _load_html(self) -> str:
        if self.source_url.startswith(("http://", "https://")):
            ...
            try:
                ...
                raise ConnectionError("Outpost Brussels inaccessible : delai depasse.") from error
            except URLError as error:
                raise ConnectionError("Outpost Brussels inaccessible : verifie l'URL source.") from error

        path = Path(self.source_url).expanduser().resolve()
        if not path.exists():
            raise FileNotFoundError(f"Snapshot Outpost Brussels introuvable: {path}")
    ...
    def _find_product_json_ld(self, scripts: list[str]) -> dict[str, Any]:
        ...
        raise ValueError("Page Outpost Brussels invalide : JSON-LD Product introuvable ou invalide.")
    ...
    def _extract_name(self, product: dict[str, Any], parser: OutpostProductParser) -> str:
        name = str(product.get("name") or "").strip()
        if not name:
            name = (parser.h1 or "").strip()

        if not name:
            raise ValueError("Page Outpost Brussels invalide : titre produit introuvable.")
        return name

    def _extract_price(self, product: dict[str, Any], parser: OutpostProductParser) -> float:
        ...
        if not raw_price:
            raise ValueError("Page Outpost Brussels invalide : prix produit introuvable.")
        return self._parse_price(raw_price)

    def _extract_stock(self, product: dict[str, Any], parser: OutpostProductParser) -> bool:
        offers = self._get_offers(product)
        ...
        if availability:
            ...
            if "outofstock" in availability or "soldout" in availability:
                return False
            raise ValueError(f"Disponibilite Outpost Brussels non reconnue: {availability}")
        ...
        raise ValueError("Page Outpost Brussels invalide : disponibilite produit introuvable.")
    ...
    def _parse_price(raw_price: str) -> float:
        ...
        if not match:
            raise ValueError(f"Prix Outpost Brussels impossible a lire: {raw_price}")
